...
## 3) Sprint 4 마일스톤 — M13: 실전 최적화 ✅ 완료 (2026-03-19)
...
| T96-H | T96 코드 리뷰 핫픽스 — Job Tracker 스켈레톤 레이아웃 불일치 수정 | ✅ 완료 |

### Phase 2: AI 자기소개서 RAG

| ID | 핵심 내용 | 상태 |
|---|---|---|
| T97 | 합격 자소서 RAG 파이프라인 (스키마 + 임베딩 + 검색) | ✅ 완료 |
| T97-H | T97 코드 리뷰 핫픽스 — 제목 120자 초과 + 스켈레톤 다크모드 | ✅ 완료 |
| T98 | 자기소개서 생성 API + 워크스페이스 UI (2세션 병렬) | ✅ 완료 (통합 게이트 최종 검증 2026-03-19) |
...
T96-H (핫픽스) ✅                   ↓
                                  ...
                                  T98 (생성 API + UI) ✅ 완료
                                    ├─ Session A: API 5개 ✅
                                    └─ Session B: UI 7개 + 사이드바 ✅
```

- **T97-H → T98**: 순차 (핫픽스 완료 후 T98 착수)
- **T98 내부**: Session A(API) ∥ Session B(UI) 병렬 (파일 무충돌)
...
**코드 리뷰 (2026-03-19)**: T96-H로 분리
- P1 수정: Job Tracker 스켈레톤 `grid-cols-6` → `flex overflow-x-auto` (실제 페이지 패턴 일치)
- P2 보류: AppShellWrapper 다크모드 FOUC → archive 이관
- P3 보류: `dark:` 접두사 이중 적용 방침 통일 → archive 이관

### T96-H — T96 코드 리뷰 핫픽스

**범위**: Job Tracker 스켈레톤 레이아웃을 실제 칸반 페이지 구조와 일치시킴

**발견**: `job-tracker/loading.tsx`가 `grid grid-cols-6`을 사용하지만 실제 `page.tsx`는 `flex gap-4 overflow-x-auto` + `w-64 min-w-[256px]` 패턴. 모바일/태블릿에서 CLS 발생 가능.

**수정 내용**:
1. `grid grid-cols-6 gap-3` → `flex gap-4 overflow-x-auto pb-4`
2. 각 열에 `w-64 min-w-[256px] flex-shrink-0` 적용
3. 실제 페이지의 `rounded-xl border-2` 카드 스타일 반영
...
**범위**: T97 코드 리뷰에서 발견된 P1/P2 이슈 수정 (T98 선행)

**수정 내용**:
1. `generateCoverLetter` 자동 제목 120자 초과 버그 — `.slice(0, 120)` 보호 (P1)
2. `job-tracker/loading.tsx` 다크모드 `bg-white` 하드코딩 → `dark:bg-zinc-900` 추가 (P2)
3. 코드 리뷰 P2+ 보류 항목 archive 이관 (6건)

### T98 — 자기소개서 생성 API + 워크스페이스 UI

**범위**: T97 RAG 파이프라인을 사용자 접점으로 노출 — API 5개 + UI 6개 + 사이드바

**2세션 병렬 구조**:

1. **Session A: API 라우트** (5개 신규)
   - `GET/POST /api/app/cover-letters` — 목록/생성
   - `GET/PATCH/DELETE /api/app/cover-letters/[id]` — 상세/수정/삭제
   - `POST /api/app/cover-letters/[id]/toggle-reference` — 합격본 토글 + 자동 임베딩
   - `POST /api/app/cover-letters/generate` — RAG 기반 AI 생성 (핵심)
   - `POST /api/app/cover-letters/[id]/embed` — 임베딩 수동 트리거

2. **Session B: 워크스페이스 UI** (6개 신규 + 1개 수정)
   - AppSidebar "자기소개서" 메뉴 추가
   - `/app/cover-letters` 목록 페이지 + loading.tsx
   - 목록 클라이언트: 카드 + AI 생성 다이얼로그 + 합격본 등록
   - `/app/cover-letters/[id]` 상세/편집 페이지 + loading.tsx
