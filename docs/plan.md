...
**범위**: T89 P1 기술 부채 해소 + 포트폴리오 공개 페이지 성능 최적화

**확정 영역** (11개 항목, 2 세션 병렬):
- Session A (P1 정리): format-resume-data.ts 공용 이동 + 편집 페이지 중복 파서 제거 + ShareLinksSection fetch 통합
- Session B (성능): 데이터 페칭 병렬화 + next/image 아바타 + loading.tsx 스켈레톤 + next.config 이미지 설정 + Lighthouse 기준선
...
### 5.4 Sprint 2 최종 결과 (2026-03-18)

- T88 ✅ + T89 ✅ + T90 ✅ — M11-P Phase 1 + Phase 2 전체 완료
- 통합 게이트 4종: lint(0 errors) / build / jest(65 suites, 429 tests) / vercel-build
- 잔여: B11 Lighthouse 기준선 + Playwright 시각 검증 (프로덕션 배포 후)
