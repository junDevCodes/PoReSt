...
## Wave3-Closure 운영 메모 (2026-02-27)

### 로컬 환경변수 운영 원칙

- `.env`는 필수이며, 팀 공통 기준값을 유지합니다.
- `.env.local`은 선택이며, 개인 오버라이드가 필요한 값만 정의합니다.
- `npm install` 시 `postinstall(prisma generate)`가 실행되므로, 최소 `DATABASE_URL`, `DATABASE_URL_UNPOOLED`, `AUTH_SECRET`는 설치 전에 준비합니다.
...
# 2) .env 필수값 입력 확인 후 Prisma Client 생성
npx prisma generate
```

### Wave3 핵심 스모크 동선 (Preview/Production 공통)

1. `/` 랜딩 진입
2. `/login` 진입 및 로그인 버튼 렌더 확인
3. `/app` 접근 확인(로그인 사용자)
4. `/app/portfolio/settings` 접근 및 미리보기 렌더 확인
5. `/app` -> `/` 이동 후 로그인 세션 유지, 홈 CTA가 `워크스페이스로 이동`을 노출하는지 확인
...
5. Wave3 핵심 스모크 동선 검증:
   - `/`
   - `/login`
   - `/app`
   - `/app/portfolio/settings`
   - `/app` -> `/` -> `/app` 이동 시 로그인 세션 유지
