...
- [완료] T30: G10 교차 링크 UI(노트/프로젝트/이력/블로그) 구현
- [완료] T31: G11 임베딩 생성 배치 파이프라인 설계
- [완료] T32: G11 임베딩 생성/재빌드 실행 API 구현
- [완료] T33: G11 유사도 검색 고도화 + 인덱스 튜닝
- [완료] T34: G12 공개 사용자 디렉토리 API 구현
- [완료] T35: G12 공개 사용자 디렉토리 UI 구현
- [완료] T36: P2 성능/비용 검증 및 운영 문서 반영

## 완료 처리 규칙
- 완료된 태스크는 `plans/history.md`에 결과(테스트/배포/리스크 포함) 기록 후 상태 전환한다.
- 섹션 단위 완료 시 커밋 메시지는 `domain(scope): 설명` 형식을 사용한다.

## Wave3 구현 트랙 (랜딩/온보딩 UX)
- [진행] T37: 루트 랜딩 페이지 재구성 + 추천/최신 쇼케이스 UI
- [진행] T38: 추천 점수 휴리스틱(`getHomeShowcase`) 구현 + 통합 테스트
- [진행] T39: 로그인/회원가입 모드 분리 + 버튼 클릭 체감 개선
- [진행] T40: 워크스페이스 → 서비스 홈 이동 경로(헤더/사이드바) 추가
- [진행] T41: 포트폴리오 설정 실시간 미리보기 + slug 경고 UX 추가
- [대기] T42: 게이트(`lint/build/jest/vercel-build`) 실행 + 문서 동기화 확정
