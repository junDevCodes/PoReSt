...
## Wave3: 랜딩/온보딩 UX + 실시간 미리보기 (2026-02-22)
목표: 루트 랜딩과 인증 진입 UX를 개선하고, 설정 화면에서 공개 포트폴리오 미리보기를 제공한다.

### 범위
1. 루트(`/`)를 서비스 랜딩 페이지로 재구성
2. 추천 Top5 + 최근 업데이트 목록 동시 노출
3. GitHub 로그인/회원가입 진입 UX 개선(버튼 클릭 체감)
4. 워크스페이스에서 서비스 홈으로 이동 경로 제공
5. 포트폴리오 설정 화면 실시간 미리보기 및 slug 변경 경고 제공

### 수용 기준
- 루트에서 서비스 소개 + 로그인/회원가입 CTA를 분리해 제공한다.
- 루트에서 추천 Top5와 최근 업데이트 목록을 동시에 노출한다.
- 로그인 버튼 클릭 즉시 로딩 상태와 비활성 상태를 보여준다.
- 워크스페이스 헤더/사이드바에서 서비스 홈으로 이동할 수 있다.
- 설정 페이지에서 미리보기가 입력값 변경에 즉시 반응한다.
- `publicSlug` 변경 시 기존 링크 깨짐(리다이렉트 없음) 경고를 명확히 표시한다.
