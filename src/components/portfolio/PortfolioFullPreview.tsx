...
/**
 * 실제 포트폴리오 공개 페이지 레이아웃과 동일하게 렌더링하는 미리보기 컴포넌트.
 * 저장 전 현재 폼 상태를 실제 크기로 확인하기 위해 settings 모달에서 사용한다.
 */
...
type PortfolioFullPreviewProps = {
  publicSlug: string;
  displayName: string;
  headline: string;
  bio: string;
...
export function PortfolioFullPreview({
  publicSlug,
  displayName,
  headline,
  bio,
  ...
  featuredResumeTitle,
  links,
}: PortfolioFullPreviewProps) {
  const resolvedName = displayName.trim() || publicSlug || "이름 미설정";
  const resolvedHeadline = headline.trim() || "헤드라인 미설정";
    ...
    <div className="min-h-screen bg-[#f6f5f2]">
      ...
      <header className="border-b border-black/8 bg-[#f6f5f2]">
        <div className="mx-auto flex h-12 w-full max-w-5xl items-center gap-2 px-6 text-sm">
          <span className="font-semibold text-black/70">PoReSt</span>
          <span className="text-black/30">/</span>
          <span className="text-black/55">{publicSlug || "slug"}</span>
        </div>
      </header>
      ...
      <main className="mx-auto flex w-full max-w-5xl flex-col px-6 py-14">
        ...
        <div className="flex items-start gap-5">
          ...
          <div>
            ...
            <h1 className="mt-1 text-4xl font-semibold">{resolvedName}</h1>
            <p className="mt-2 text-lg text-black/75">{resolvedHeadline}</p>
            <div className="mt-2 flex flex-wrap items-center gap-3">
              ...
              ) : availabilityStatus === "CONSIDERING" ? (
              ...
              {location.trim() ? (
                <span className="flex items-center gap-1 text-xs text-black/65">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="h-3.5 w-3.5">
                    <path fillRule="evenodd" d="M11.955 5.682a4 4 0 1 0-7.91 0L8 14.25l3.955-8.568ZM8 8.5a2 2 0 1 0 0-4 2 2 0 0 0 0 4Z" clipRule="evenodd" />
                  </svg>
                  {location}
                </span>
              ) : null}
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          {hasResume ? (
            <span className="rounded-full border border-black/20 px-5 py-3 text-sm font-semibold text-black/80">
        ...
        {sortedLinks.length > 0 ? (
          <section className="mt-6 flex flex-wrap gap-3">
            {sortedLinks.map((link, i) => (
              <span
                key={`${link.label}-${i}`}
                className="flex items-center gap-1.5 rounded-full border border-black/20 px-4 py-2 text-sm text-black/75"
              >
