...
type PortfolioSettingsPageClientProps = {
  initialSettings: PortfolioSettingsDto | null;
  initialResumes: ResumeListItem[];
};

export function PortfolioSettingsPageClient({
  initialSettings,
  initialResumes,
}: PortfolioSettingsPageClientProps) {
  const [form, setForm] = useState<PortfolioSettingsFormState>(() => toFormState(initialSettings));
  const [originalPublicSlug, setOriginalPublicSlug] = useState<string | null>(
    initialSettings?.publicSlug ?? null,
  );
  const [savedIsPublic, setSavedIsPublic] = useState(initialSettings?.isPublic ?? false);
  const [savedPublicSlug, setSavedPublicSlug] = useState(initialSettings?.publicSlug ?? "");
  const [isSaving, setIsSaving] = useState(false);
  const [isUploadingAvatar, setIsUploadingAvatar] = useState(false);
  const [avatarUploadError, setAvatarUploadError] = useState<string | null>(null);
  const [isUploadingResume, setIsUploadingResume] = useState(false);
  const [resumeUploadError, setResumeUploadError] = useState<string | null>(null);
  const [resumeMode, setResumeMode] = useState<"upload" | "internal">(
    initialSettings?.featuredResumeId ? "internal" : "upload",
  );
  const [availableResumes, setAvailableResumes] = useState<ResumeListItem[]>(initialResumes);
  const [featuredResumeTitle, setFeaturedResumeTitle] = useState(
    initialSettings?.featuredResumeTitle ?? "",
  );
