...
type BlogPostsPageClientProps = {
  initialPosts: SerializedOwnerBlogPostListItemDto[];
};
...
export function BlogPostsPageClient({ initialPosts }: BlogPostsPageClientProps) {
  const [posts, setPosts] = useState<SerializedOwnerBlogPostListItemDto[]>(initialPosts);
  const [isLoading, setIsLoading] = useState(false);
  const [workingId, setWorkingId] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [pendingDeletePost, setPendingDeletePost] =
    useState<SerializedOwnerBlogPostListItemDto | null>(null);
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col">
      ...
      <section className="mt-8 rounded-2xl border border-black/10 bg-white p-6">
        ...
        {isLoading ? (
          <LoadingBlock message="글 목록을 불러오는 중입니다." className="mt-4" />
        ) : posts.length === 0 ? (
          <EmptyBlock message="등록된 글이 없습니다." className="mt-4" />
        ) : (
