...
type ResumeEditPageClientProps = {
  resumeId: string;
  initialResume: OwnerResumeDetailDto;
  initialExperiences: OwnerExperienceOptionDto[];
};

export function ResumeEditPageClient({
  resumeId,
  initialResume,
  initialExperiences,
}: ResumeEditPageClientProps) {
  const router = useRouter();
  const [resume, setResume] = useState<OwnerResumeDetailDto>(initialResume);
  const [form, setForm] = useState<ResumeFormState>(toResumeFormState(initialResume));
  const [itemEditors, setItemEditors] = useState<Record<string, ItemEditor>>(toItemEditors(initialResume.items));
  const [availableExperiences, setAvailableExperiences] = useState<OwnerExperienceOptionDto[]>(initialExperiences);
  ...
  const outdatedItemCount = useMemo(() => {
    return resume.items.filter((item) => {
        ...
        <div className="mt-4 grid gap-4">
          ...
          <label className="flex flex-col gap-2 text-sm">
            ...
            <textarea
              ...
              onChange={(event) =>
                setForm((prev) => ({ ...prev, summaryMd: event.target.value }))
              }
