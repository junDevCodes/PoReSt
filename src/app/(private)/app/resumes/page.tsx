...
export default function ResumesPage() {
  ...
  const [pendingDeleteResume, setPendingDeleteResume] = useState<OwnerResumeListItemDto | null>(null);
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col">
      ...
      <section className="mt-8 rounded-2xl border border-black/10 bg-white p-6">
        ...
        {isLoading ? (
          <p className="mt-4 text-sm text-black/60">이력서 목록을 불러오는 중입니다.</p>
        ) : resumes.length === 0 ? (
          <p className="mt-4 text-sm text-black/60">등록된 이력서가 없습니다.</p>
        ) : (
