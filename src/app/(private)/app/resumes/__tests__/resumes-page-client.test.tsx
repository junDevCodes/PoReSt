...
describe("Test-M6-03 ResumesPageClient", () => {
  beforeEach(() => {
  ...
  it("삭제 후 목록 재조회 API 실패 시 로딩을 해제하고 오류 배너를 표시해야 한다", async () => {
    const fetchMock = jest
      .fn()
      .mockResolvedValueOnce(buildMockFetchResponse({ data: { id: "resume-1" } }))
      .mockRejectedValueOnce(new TypeError("fetch failed"));
    global.fetch = fetchMock as unknown as typeof fetch;

    render(<ResumesPageClient initialResumes={INITIAL_RESUMES} />);
    ...
    expect(screen.queryByText("이력서 목록을 불러오는 중입니다.")).not.toBeInTheDocument();
