...
import type {
  SerializedOwnerCompanyTargetDto,
...
function buildEditors(items: SerializedOwnerCompanyTargetDto[]): Record<string, TargetEditor> {
  return items.reduce<Record<string, TargetEditor>>((acc, item) => {
    acc[item.id] = {
      company: item.company,
      role: item.role,
      status: item.status as CompanyTargetStatus,
...
type CompanyTargetsPageClientProps = {
  initialTargets: SerializedCompanyTargetsListResult;
};

export function CompanyTargetsPageClient({ initialTargets }: CompanyTargetsPageClientProps) {
  const [targets, setTargets] = useState<SerializedOwnerCompanyTargetDto[]>(initialTargets.items);
  const [editors, setEditors] = useState<Record<string, TargetEditor>>(() =>
    buildEditors(initialTargets.items),
  );
  ...
  function applyTargets(items: SerializedOwnerCompanyTargetDto[]) {
    setTargets(items);
  ...
  async function requestTargets() {
    ...
    try {
      ...
      return await parseApiResponse<SerializedCompanyTargetsListResult>(response);
    } catch (error) {
      return parseApiResponse<SerializedCompanyTargetsListResult>(error);
    }
  }
  ...
  async function handleCreate(event: FormEvent) {
    ...
    const parsed = await (async () => {
      try {
        ...
        return await parseApiResponse<SerializedOwnerCompanyTargetDto>(response);
      } catch (error) {
        return parseApiResponse<SerializedOwnerCompanyTargetDto>(error);
      }
    })();
    if (parsed.error) {
      setError(parsed.error);
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-black/60">
                          상태: {STATUS_LABELS[target.status as CompanyTargetStatus]} | 우선순위:{" "}
                          {target.priority}
                        </p>
                      </div>
