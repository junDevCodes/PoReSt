...
export type JobCardDetailModalProps = {
  ...
  onStatusChange: (targetId: string, newStatus: CompanyTargetStatus) => void;
        ...
        <div className="flex items-start justify-between border-b border-black/10 p-6">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-semibold">{card.company}</h2>
              <span className={`rounded px-2 py-0.5 text-xs font-medium ${STATUS_BADGE_COLORS[card.status]}`}>
                {STATUS_LABELS[card.status]}
              </span>
            </div>
            <p className="mt-1 text-sm text-black/60">{card.role}</p>
            {card.appliedAt && (
              <p className="mt-1 text-xs text-black/40">
                지원일: {new Date(card.appliedAt).toLocaleDateString("ko-KR")}
              </p>
            )}
          </div>
          <button
          ...
          <div className="grid gap-6">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {STATUS_ORDER.filter((s) => s !== card.status).map((status) => (
                  <button
                    key={status}
                    type="button"
            ...
            {card.summary && (
              <div>
                ...
                <p className="mt-1 text-sm text-black/70 whitespace-pre-wrap">{card.summary}</p>
              </div>
            )}
