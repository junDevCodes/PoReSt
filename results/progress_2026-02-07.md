...
## 13차 업데이트 (M2 문서 마감 + 다음 단계 고정)
- M2 마감 문서 동기화
  - `plans/checklist.md`에 M2 문서/배포 체크 항목 반영
  - `plans/task.md`에 M2 운영/문서 마감 항목 반영
- 다음 실행 순서 확정
  - M3 -> M4 -> M5 순으로 구현/검증/배포 진행
  - M1.1 Deferred 및 세부 UI/UX 개선은 M5 완료 이후로 이관
- 운영 기준 재확인
  - 게이트: `npm run lint`, `npm run build`, `npx jest --runInBand`
  - 배포: `Preview -> 수동 스모크 -> Production`

## 14차 업데이트 (M3 Notes 1차 백엔드 착수)
- Notes 모듈 신규 추가
  - `src/modules/notes/interface.ts`
  - `src/modules/notes/implementation.ts`
  - `src/modules/notes/http.ts`
  - `src/modules/notes/index.ts`
- Notes API 신규 추가
  - `GET/POST /api/app/notes`
  - `GET/PUT/DELETE /api/app/notes/[id]`
  - `GET /api/app/notes/search`
  - `GET /api/app/notes/edges`
  - `POST /api/app/notes/edges/confirm`
  - `POST /api/app/notes/edges/reject`
- Candidate Generator 1차 구현
  - 태그 기반 Jaccard 유사도 계산
  - threshold 0.7, Top-N 제한, 중복 후보 제거, 자기 자신 제외
  - 후보 엣지 생성 후 confirm/reject 상태 전이
- 테스트 추가
  - `src/modules/notes/tests/validation.test.ts`
  - `src/modules/notes/tests/implementation.integration.test.ts`
- 게이트 실행 결과
  - `npx jest src/modules/notes/tests/validation.test.ts --runInBand` 통과
  - `npx jest src/modules/notes/tests/implementation.integration.test.ts --runInBand` 통과
  - `npm run lint` 통과
  - `npm run build` 통과
- `.env.local` 로드 후 `npx jest --runInBand` 통과 (`18 suites`, `64 tests`)

## 15차 업데이트 (M3 Candidate Domain 가중치 반영)
- Domain 필터링/가중치 로직 구현
  - 파일: `src/modules/notes/implementation.ts`
  - 규칙: 동일 노트북(도메인) 노트 쌍에 후보 엣지 가중치 보너스 적용
  - 보정: `weight = min(1, jaccard + bonus)` 형태로 계산
- 회귀 확인
  - `src/modules/notes/tests/implementation.integration.test.ts` 통과
  - `동일 도메인(노트북) 노트 쌍은 가중치 보정을 받아야 한다` 시나리오 통과

## 16차 업데이트 (M3 Notes 목록 UI 착수)
- `/app/notes` 페이지 추가
  - 파일: `src/app/(private)/app/notes/page.tsx`
  - 노트북별 그룹 목록 + 노트 목록 렌더
  - 노트 상세 이동 링크(`/app/notes/[id]`) 추가
- 목록 가공 유틸/테스트 추가
  - 유틸: `src/app/(private)/app/notes/_lib/list.ts`
  - 테스트: `src/app/(private)/app/notes/_lib/__tests__/notes-list.test.ts`
  - 검증: 그룹핑/수정일 정렬 시나리오 통과
- 대시보드 빠른 링크 확장
  - 파일: `src/app/(private)/app/page.tsx`
  - `/app/notes` 링크 추가
- 게이트 확인
  - `npx jest --runInBand notes-list.test.ts` 통과
  - `npm run lint` 통과
  - `npm run build` 통과

## 17차 업데이트 (M3 노트 상세 + 인덱스 최적화)
- 노트 상세 페이지 추가
  - 파일: `src/app/(private)/app/notes/[id]/page.tsx`
  - 파일: `src/app/(private)/app/notes/[id]/NoteDetailClient.tsx`
  - 기능: CONFIRMED/CANDIDATE 분리 표시, Confirm/Reject 액션
- 노트 상세 유틸/테스트 추가
  - 유틸: `src/app/(private)/app/notes/_lib/detail.ts`
  - 테스트: `src/app/(private)/app/notes/_lib/__tests__/note-detail.test.ts`
- 노트 상세용 API 추가
  - 라우트: `GET /api/app/notes/[id]/edges`
  - 파일: `src/app/api/app/notes/[id]/edges/route.ts`
  - 서비스: `listEdgesForNoteForOwner` 메서드 추가
- M3 성능 최적화(인덱스) 반영
  - 스키마: `prisma/schema.prisma` (`Note`, `NoteEdge` 인덱스 확장)
  - 마이그레이션: `prisma/migrations/20260211193000_m3_notes_index_tuning/migration.sql`
- 검증 결과
  - `src/modules/notes/tests/implementation.integration.test.ts` 통과
  - `src/app/(private)/app/notes/_lib/__tests__/note-detail.test.ts` 통과
  - `npm run lint` 통과

## 18차 업데이트 (M3 그래프 시각화 반영)
- 노트 그래프 유틸/테스트 추가
  - 유틸: `src/app/(private)/app/notes/_lib/graph.ts`
  - 테스트: `src/app/(private)/app/notes/_lib/__tests__/graph.test.ts`
- 노트 상세 그래프 UI 반영
  - 파일: `src/app/(private)/app/notes/[id]/NoteDetailClient.tsx`
  - 기능: 중심 노트 기준 연결 노트/엣지 SVG 시각화, 상태별 색상 구분
- 검증 결과
  - `npx jest graph.test.ts --runInBand` 통과
  - `npx jest note-detail.test.ts --runInBand` 통과
  - `.env.local` 로드 후 `src/modules/notes/tests/implementation.integration.test.ts` 통과
  - `npm run lint` 통과

## 19차 업데이트 (M3 핵심 마감 확정)
- M3 마감 기준 반영
  - `plans/task.md`: M3 상태를 `핵심 마감`으로 갱신, `M3 구현/검증/배포` 완료 처리
  - `plans/checklist.md`: `M3 완료` 체크 반영
- 게이트 재검증
  - `.env.local` 로드 후 `npx jest --runInBand` 통과 (`21 suites`, `71 tests`)
  - `npm run lint` 통과
  - `npm run build` 통과
- 비고
  - `pgvector 파이프라인`은 선택 항목으로 `M3.1 Deferred`로 분리 유지
  - Windows 환경에서 `next build` 중 `spawn EPERM`이 간헐 재현되며 재실행 시 정상 완료
